...
//!
//! Fonts are stored as row-major bitmaps where each row is a byte.
//! Bits represent pixels from left to right within the character width.

/// Bitmap font descriptor (row-based format)
...
impl BitmapFont {
    /// Get glyph rows for a character (returns array of row bytes)
    pub fn get_glyph(&self, ch: char) -> Option<&'static [u8]> {
        let code = ch as u32;
        ...
        if index < FONT_6X10_ROWS.len() {
            Some(&FONT_6X10_ROWS[index])
        } else {
            None
        }
    }
...
// Primary font: 6x10 (simple readable font)
...
// Secondary font: 5x7 (compact) - uses primary for now
pub static FONT_SECONDARY: BitmapFont = BitmapFont {
    width: 6,
    height: 10,
    baseline: 8,
    first_char: 32,
...
// Keyboard font: 5x8 - uses primary for now
pub static FONT_KEYBOARD: BitmapFont = BitmapFont {
    width: 6,
    height: 10,
    baseline: 8,
    first_char: 32,
...
// Big numbers: 10x20 - uses primary for now
pub static FONT_BIG_NUMBERS: BitmapFont = BitmapFont {
    width: 6,
    height: 10,
    baseline: 8,
    first_char: 32,
...
/// 6x10 font with printable ASCII (32-126)
/// Each row is a byte where bits 6-1 are the 6 pixels (left to right)
/// The font data is shifted left by 2 bits for 7-bit wide rendering
#[rustfmt::skip]
pub static FONT_6X10_ROWS: [[u8; 10]; 95] = [
    // Space (32)
    ...
    // & (38)
    [0x00, 0x30, 0x48, 0x48, 0x30, 0x54, 0x48, 0x34, 0x00, 0x00],
    // ' (39)
    ...
    // y (121)
    [0x00, 0x00, 0x00, 0x44, 0x44, 0x44, 0x3C, 0x04, 0x38, 0x00],
    // z (122)

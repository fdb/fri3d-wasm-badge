...
pub fn footerLeft(text: [:0]const u8) bool {
    canvas.setFont(.secondary);
    ...
    return g_ctx.has_input and
        g_ctx.last_key == .left and
        (g_ctx.last_type == .short_press or g_ctx.last_type == .press);
}

/// Center footer button (OK + text) - returns true on OK+Short (if not focused elsewhere)
pub fn footerCenter(text: [:0]const u8) bool {
    canvas.setFont(.secondary);
...
pub fn footerRight(text: [:0]const u8) bool {
    canvas.setFont(.secondary);
